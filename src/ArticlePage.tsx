    ...
    <main className="article-page">
      ...
      <article>
        <header className="article-header">
          <a className="article-category" href={`/insights#${article.category === 'Club Strategy & Operations' ? 'club-strategy' : 'coaching'}`}>{article.category}</a>
          <h1>{article.title}</h1>
          <div className="article-meta"><span>By Julian Wortelboer</span><span>{article.date}</span><span>{article.readTime}</span></div>
        </header>
        ...
        <div className="article-layout">
          ...
          <div className="article-body">
            ...
            {article.category === 'Coaching & Player Development' && <a className="text-link article-academy-link" href={coachingCertificationUrl} target="_blank" rel="noreferrer">Explore Padel Coach Certification <ArrowRight size={17} /></a>}
